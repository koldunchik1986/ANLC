...
object AuthManager {
    ...
    private fun logToFile(context: android.content.Context, text: String) {
        try {
            val file = java.io.File(context.filesDir, "log.txt")
    ...
    fun authorizeAsync(
        ...
        GlobalScope.launch(Dispatchers.IO) {
            try {
                ...
                context?.let { logToFile(it, "GET $GAME_URL\nCOOKIES (sent):\n$cookieHeader\nCOOKIES (all):\n$allCookies\nSET-COOKIE:\n${getCookies.joinToString("\n")}") }
                ...
                postResponse.close()
                context?.let { logToFile(it, "POST $LOGIN_URL\nPARAMS: player_nick=$login, player_password=***\nCOOKIES (sent):\n$cookieHeader\nCOOKIES (all):\n$allCookiesPost\nSET-COOKIE:\n${postCookies.joinToString("\n")}") }
                ...
                context?.let { logToFile(it, "GET ${GAME_URL}main.php\nCOOKIES (sent):\n$authCookies\nCOOKIES (all):\n$allCookiesMain\nRESPONSE (decoded):\n$mainBody") }

                // Расширенная проверка признаков формы авторизации
                val isAuthForm = mainBody.contains("auth_form", true) ||
                                 mainBody.contains("action=\"./game.php\"", true) ||
                                 mainBody.contains("form name=\"auth\"", true) ||
                                 (mainBody.contains("логин", true) && mainBody.contains("пароль", true)) ||
                                 (mainBody.contains("login", true) && mainBody.contains("password", true)) ||
                                 mainBody.contains("введите", true) ||
                                 mainBody.contains("неверный пароль", true) ||
                                 mainBody.contains("ошибка авторизации", true)

                if (isAuthForm) {
                    callback(1, null, "Неверный логин или пароль")
                } else if (mainBody.contains("captcha", true)) {
                    callback(2, null, "Требуется капча")
                } else if (mainBody.isNotBlank()) {
                    callback(0, mainBody, null)
                } else {
                    callback(3, null, "Ошибка сети или пустой ответ")
                }
            } catch (e: Exception) {
                context?.let { logToFile(it, "AUTH ERROR: ${e.message}") }
                callback(3, null, "Ошибка: ${e.message}")
            }
        }
    }
}
