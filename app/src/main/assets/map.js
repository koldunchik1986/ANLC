...
function StateReady()
{
    ...
    switch(arr_res[0])
    {
        ...
        // +ABC
        var id = "td_" + gox + "_" + goy;
        d.getElementById(id).innerHTML = window.external.CellDivText(gox, goy, scale, '', true, false);
        ...
        case "bottom":
            ...
            for (i = loaded_left; i <= loaded_right; i++) {
                ...
                if (window.external.IsCellExists(i, loaded_bottom)) {
                    ...
                    td.innerHTML = window.external.CellDivText(dx, dy, scale, td.onclick, false, false);
                }
                else {
                    td.style.opacity = "0.4";
                    td.style.filter = "alpha(opacity=40)";
...
function createMapText() {
    ...
    div.innerHTML = "<table cellpadding=0 cellspacing=0 border=0 width=" + abcmapwidth + "><tr><td align=center id=\"maptext\" style=\"font-family:Verdana; font-size:12px; color:#999999; font-weight:bold; text-shadow:2px 2px 5px black;\"></td></tr></table>";
